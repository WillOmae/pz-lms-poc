...
CREATE TABLE lms.transaction_type_components (
    id                  BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    name                TEXT                     NOT NULL UNIQUE,
    description         TEXT                     NOT NULL,
    transaction_type_id BIGINT REFERENCES lms.transaction_types ON UPDATE RESTRICT ON DELETE CASCADE,
    execution_order     INT                      NOT NULL,
    permission_id       BIGINT REFERENCES lms.permissions ON UPDATE RESTRICT ON DELETE RESTRICT,
    debit_account_id    BIGINT REFERENCES lms.accounts ON UPDATE RESTRICT ON DELETE CASCADE,
    credit_account_id   BIGINT REFERENCES lms.accounts ON UPDATE RESTRICT ON DELETE CASCADE,
    date_created        TIMESTAMP WITH TIME ZONE NOT NULL,
    date_updated        TIMESTAMP WITH TIME ZONE
);
...
CREATE TABLE lms.transactions (
    ...
    initiator             BIGINT REFERENCES lms.entities ON UPDATE RESTRICT ON DELETE RESTRICT,
    debit_entity_id       BIGINT REFERENCES lms.entities ON UPDATE RESTRICT ON DELETE RESTRICT,
    credit_entity_id      BIGINT REFERENCES lms.entities ON UPDATE RESTRICT ON DELETE RESTRICT,
...
CREATE TABLE lms.transaction_entries (
    id                            BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    transaction_reference         TEXT                     NOT NULL UNIQUE,
    transaction_type_component_id BIGINT                   NOT NULL REFERENCES lms.transaction_type_components ON UPDATE RESTRICT ON DELETE RESTRICT,
    amount_minor                  BIGINT                   NOT NULL,
    currency                      BIGINT                   NOT NULL REFERENCES lms.currencies ON UPDATE RESTRICT ON DELETE RESTRICT,
    date_created                  TIMESTAMP WITH TIME ZONE NOT NULL,
    date_updated                  TIMESTAMP WITH TIME ZONE
...
CREATE TABLE lms.loan_products (
    ...
    name                           TEXT                     NOT NULL UNIQUE,
    description                    TEXT                     NOT NULL,
    currency_code                  CHAR(3)                  NOT NULL REFERENCES lms.currencies (code) ON UPDATE RESTRICT ON DELETE RESTRICT,
    min_loan_amount_minor          BIGINT                   NOT NULL,
    max_loan_amount_minor          BIGINT                   NOT NULL,
    ...
    access_fee_amount_minor        BIGINT,
    ...
    date_created                   TIMESTAMP WITH TIME ZONE NOT NULL,
    date_updated                   TIMESTAMP WITH TIME ZONE,
    CHECK (access_fee_chargeable = FALSE OR ((access_fee_amount_minor IS NOT NULL AND access_fee_percentage IS NULL) OR
                                             (access_fee_amount_minor IS NULL AND access_fee_percentage IS NOT NULL)))
);
...
CREATE TABLE lms.product_transaction_types (
    id                  BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    loan_product_id     BIGINT                   NOT NULL REFERENCES lms.loan_products ON UPDATE RESTRICT ON DELETE RESTRICT,
    transaction_type_id BIGINT                   NOT NULL REFERENCES lms.transaction_types ON UPDATE RESTRICT ON DELETE RESTRICT,
    date_created        TIMESTAMP WITH TIME ZONE NOT NULL,
    date_updated        TIMESTAMP WITH TIME ZONE,
    CONSTRAINT unique_product_transaction_type UNIQUE (loan_product_id, transaction_type_id)
);
...
CREATE TRIGGER trg_audit_transaction_type_components
    AFTER INSERT OR UPDATE OR DELETE
    ON lms.transaction_type_components
    FOR EACH ROW
EXECUTE PROCEDURE lms.audit_trigger();
...
CREATE TRIGGER trg_audit_product_transaction_types
    AFTER INSERT OR UPDATE OR DELETE
    ON lms.product_transaction_types
    FOR EACH ROW
EXECUTE PROCEDURE lms.audit_trigger();
